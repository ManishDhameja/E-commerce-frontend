...
export default function OrderHistory() {
  ...
  const { orderHistory, loading: historyLoading } = useSelector(
    ...
    <Box sx={{ p: (theme) => theme.spacing(4) }}>
      <Grid container justifyContent="space-between" alignItems="center">
        ...
        {historyLoading ? (
          ...
          <Grid item>
            <Typography variant="h4">{orderHistory.length} items</Typography>
          </Grid>
        )}
      </Grid>
          ...
          orderHistory
            ...
            .map((item, i) => {
              return lessThan800 ? (
                ...
                <CartItem {...item} img={item.imgs[2]} key={i} isOrder />
              );
            })
        )}
      </Stack>
